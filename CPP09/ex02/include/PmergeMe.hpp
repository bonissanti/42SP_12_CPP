/*************************************************************************************/
/*   ⠀⠀  ⠀⠀⠀⠀ ⣴⣿⣦ ⠀⠀⠀⠀⠀⠀⠀⠀                                                       */
/* ⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢻⣿⣿⠂⠀⠀⠀⠀⠀⠀⠀⠀                                                       */
/* ⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⣴⣿⣿⣀⠀⠀⠀⠀⠀⠀⠀⠀                                                       */
/* ⠀⠀⠀⠀⠀⠀⠀⢠⣾⣿⣿⣿⣿⣿⣿⣦          ⠀                                                   */
/* ⠀⠀⠀⠀⠀⠀⣴⣿⢿⣷⠒⠲⣾⣾⣿⣿⠂         Created by: brunrodr - 06/22/2024                   */
/* ⠀⠀⠀⠀⣴⣿⠟⠁⠀⢿⣿⠁⣿⣿⣿⠻⣿⣄⠀⠀⠀⠀   Updated by: brunrodr - 06/22/2024                   */
/* ⠀⠀⣠⡾⠟⠁⠀⠀⠀⢸⣿⣸⣿⣿⣿⣆⠙⢿⣷⡀⠀⠀                                                       */
/* ⣰⡿⠋⠀⠀⠀⠀⠀⠀⢸⣿⣿⣿⣿⣿⣿⠀⠀⠉⠻⣿⡀                                                       */
/* ⠀⠀⠀⠀⠀⠀⠀⠀⠀⣾⣿⣿⣿⣿⣿⣿⣆ ⠀       Email: brunrodr@student.42sp.org.br                 */
/* ⠀⠀⠀⠀⠀⠀⠀⠀⣼⣿⣿⣿⡿⣿⣿⣿⣿⡄⠀⠀⠀⠀                                                       */
/* ⠀⠀⠀⠀⠀⠀⠀⢠⣿⣿⠿⠟⠀⠀⠻⣿⣿⡇⠀⠀⠀⠀   ███████╗██╗██╗██╗██╗██╗██╗██╗██╗██╗   ██╗██╗   ██╗  */
/* ⠀⠀⠀⠀⠀⠀⢀⣾⡿⠃⠀⠀⠀⠀⠀⠘⢿⣿⡀⠀⠀⠀   ██╔════╝██║██║██║██║██║██║██║██║██║   ██║██║   ██║  */
/* ⠀⠀⠀⠀⠀⣰⣿⡟⠀⠀⠀⠀⠀⠀⠀⠀⢸⣿⣷⡀⠀⠀   ███████╗██║██║██║██║██║██║██║██║██║   ██║██║   ██║  */
/* ⠀⠀⠀⠀⢠⣿⠟⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠻⣿⣧⠀⠀   ╚════██║██║██║██║██║██║██║██║██║██║   ██║██║   ██║  */
/* ⠀⠀⠀⢀⣿⠃⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠘⣿⣆⠀   ███████║██║██║██║██║██║██║██║██║╚██████╔╝╚██████╔╝  */
/*  ⠀⠠⢾⠇⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢸⣿⣷⡤  ╚══════╝╚═╝╚═╝╚═╝╚═╝╚═╝╚═╝╚═╝╚═╝ ╚═════╝  ╚═════╝   */
/*************************************************************************************/

#ifndef PMERGEME_HPP
# define PMERGEME_HPP

#include <iostream>
#include <vector>
#include <deque>
#include <algorithm>
#include <iterator>
#include <utility>
#include <sstream>
#include <iterator>

#define RED	"\033[0;31m"
#define BYELLOW	"\033[1;33m"
#define YELLOW	"\033[0;33m"
#define PURPLE	"\033[0;34m"
#define BBLUE	"\033[1;34m"
#define BWHITE	"\033[1;37m"
#define BLACK	"\033[1;30m"
#define GREEN	"\033[0;32m"
#define RESET	"\033[0m"

class PmergeMe
{
	private:
	std::vector<int> vec;
	std::deque<int> dqe;
	std::vector<int> jacob;
	std::vector<int> insertOrder;
	clock_t start;
	clock_t end;

	public:
	PmergeMe(int argc, char **argv);
	~PmergeMe();
	PmergeMe(const PmergeMe& toCopy);
	PmergeMe& operator=(const PmergeMe& toCopy);

	void	runVector(void);
	void	runDeque(void);
	std::vector<int> convertVector(int argc, char **argv);
	std::deque<int> convertDeque(int argc, char **argv);
	
	template <typename T>
	void	printContainer(T& container, const std::string& msg, const std::string& color);

	template <typename T>
	void	printTime(T&container, const std::string& msg);

	template <typename T>
	T mergeSort(T& container);

	template <typename T>
	T merge(T& leftContainer, T& rightContainer);

	template <typename T>
	void insertionSort(T& mainChain, T& pend);

	template <typename T>
	void	generateinsertOrder(T& pend);

	class isSortedException : public std::exception
	{
		private:
		std::string msg;
		
		public:
		isSortedException(const std::string& msg);
		virtual ~isSortedException() throw();
		virtual const char* what() const throw();
	};

	class invalidDigitsException : public std::exception
	{
		private:
		std::string msg;

		public:
		invalidDigitsException(const std::string& msg);
		virtual ~invalidDigitsException() throw();
		virtual const char* what() const throw();
	};
};

bool	validDigits(int argc, char **argv);
void	debugMode(const std::string& msg);

#endif